...
$(function(){
	$('#menu').tree({
		onClick: function(node){ /* onClick针对的是整棵树 */
			if($('#menu').tree("isLeaf",node.target)){  /* 判断点击的是否是叶子节点 */
				var tabs = $("#tabs");
				var tab = tabs.tabs("getTab",node.text);
				if(tab){ /* 判断右侧的tab是否存在 */
					tabs.tabs("select",node.text); /*存在，则选中  */
				}else{
					tabs.tabs('add',{  /*不存在，创建  */
					    title:node.text,
					    href: node.attributes.url, /* 每个菜单对应自己的一个地址，data-option */
					    closable:true,
					    bodyCls:"content"
					});
				}
